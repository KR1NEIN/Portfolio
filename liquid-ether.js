 ...
 * Converted from React to vanilla JS for static HTML
 */
...
(function() {
  ...
  const config = {
    ...
    resolution: 0.35,
    isBounce: false,
    colors: ['#e8a838', '#b88828', '#5227FF'], // Gold + Blue theme matching design
    autoDemo: true,
    autoSpeed: 0.5,
    autoIntensity: 2.2,
  ...
  class CommonClass {
    ...
    init(container) {
      ...
      this.pixelRatio = 1; // Force pixel ratio 1 for performance - fluid sim doesn't need HiDPI
      this.resize();
  ...
  class MouseClass {
    ...
    init(container) {
      this.container = container;
      this.docTarget = container.ownerDocument || document;
    ...
    dispose() {
      if (this.listenerTarget) {
        this.listenerTarget.removeEventListener('mousemove', this.onDocumentMouseMove.bind(this));
        this.listenerTarget.removeEventListener('touchstart', this.onDocumentTouchStart.bind(this));
        this.listenerTarget.removeEventListener('touchmove', this.onDocumentTouchMove.bind(this));
        this.listenerTarget.removeEventListener('touchend', this.onTouchEnd.bind(this));
      }
      if (this.docTarget) {
        this.docTarget.removeEventListener('mouseleave', this.onDocumentLeave.bind(this));
      }
    }
  ...
  class ExternalForce extends ShaderPass {
    ...
    update(props) {
      ...
      // Skip rendering when there's no mouse force to apply
      if (forceX === 0 && forceY === 0 && !Mouse.mouseMoved) return;
  ...
  class WebGLManager {
    constructor(props) {
      ...
      this._frameTimes = [];
      this._adaptiveIterations = config.iterationsPoisson;
    ...
    // Adaptively reduce iterations when frame rate drops
    adaptQuality() {
      const now = performance.now();
      ...
      // Only adapt every 30 frames
      if (this._frameTimes.length < 30) return;
      const avgFrameTime = this._frameTimes.reduce((a, b) => a + b, 0) / this._frameTimes.length;
      const targetFrameTime = 20; // ~50fps target
      const sim = this.output?.simulation;
      if (!sim) return;
      if (avgFrameTime > targetFrameTime * 1.5 && this._adaptiveIterations > 8) {
        // FPS too low, reduce iterations
        this._adaptiveIterations = Math.max(8, this._adaptiveIterations - 2);
        sim.options.iterations_poisson = this._adaptiveIterations;
        this._frameTimes = [];
      } else if (avgFrameTime < targetFrameTime * 0.7 && this._adaptiveIterations < config.iterationsPoisson) {
        // FPS high enough, restore iterations
        this._adaptiveIterations = Math.min(config.iterationsPoisson, this._adaptiveIterations + 1);
        sim.options.iterations_poisson = this._adaptiveIterations;
        this._frameTimes = [];
      }
    }
  ...
  window.initLiquidEther = function(containerId) {
  ...
  // Cleanup function
  window.destroyLiquidEther = function() {
